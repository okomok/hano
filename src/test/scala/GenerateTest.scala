...
class GenerateTest extends org.scalatest.junit.JUnit3Suite {
    ...
    def testThen: Unit = {
        ...
        for (x <- hano.Context.act.loop.generate(hano.Iter(9,8,7,6,5)).onExit(_ => {s.add(99);i.countDown})) {
            s.add(x)
            i.countDown()
        }
        i.await()
    ...
    def testThenAppend: Unit = {
        ...
        for (x <- hano.Context.act.loop.generate(hano.Iter(9,8,7,6,5)) ++ hano.Seq(2,3,4)) {
            s.add(x)
            i.countDown()
        }
        i.await()
