...
class SplitAtTest extends org.scalatest.junit.JUnit3Suite {
    ...
    def testTrivial {
        ...
        ys.foreach(ya.add(_))
        assertEquals(hano.util.Iterable(1,2), hano.util.Iterable.from(xa))
        assertEquals(hano.util.Iterable(3,4,5), hano.util.Iterable.from(ya))
    }
    ...
    def testClosed {
        ...
        assertTrue(r.closed)
        assertEquals(hano.util.Iterable(1), hano.util.Iterable.from(xa))
        assertEquals(hano.util.Iterable(3,4), hano.util.Iterable.from(ya))
    }
    ...
    def testClosed2 {
        ...
        assertTrue(r.closed)
        assertEquals(hano.util.Iterable(1,2), hano.util.Iterable.from(xa))
        assertEquals(hano.util.Iterable(3), hano.util.Iterable.from(ya))
    }
