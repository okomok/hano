...
class SplitAtTest extends org.scalatest.junit.JUnit3Suite {
    ...
    def testTrivial {
        ...
        ys.foreach(ya.add(_))
        assertEquals(hano.util.Vector(1,2), hano.util.Vector.from(xa))
        assertEquals(hano.util.Vector(3,4,5), hano.util.Vector.from(ya))
    }
    ...
    def testClosed {
        ...
        assertTrue(r.closed)
        assertEquals(hano.util.Vector(1), hano.util.Vector.from(xa))
        assertEquals(hano.util.Vector(3,4), hano.util.Vector.from(ya))
    }
    ...
    def testClosed2 {
        ...
        assertTrue(r.closed)
        assertEquals(hano.util.Vector(1,2), hano.util.Vector.from(xa))
        assertEquals(hano.util.Vector(3), hano.util.Vector.from(ya))
    }
