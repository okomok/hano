...
class ZipTest extends org.scalatest.junit.JUnit3Suite {
    def testTrivial: Unit = {
        ...
        assertEquals(hano.util.Iterable((1,"2"),(2,"3"),(3,"4")), hano.util.Iterable.from(out))
    }
    ...
    def testOneShorter1: Unit = {
        ...
        val out = new java.util.ArrayList[(Int, String)]
        t.zip(u).foreach(out.add(_))
        assertEquals(hano.util.Iterable((1,"2"),(2,"3")), hano.util.Iterable.from(out))
    }
    ...
    def testParallel: Unit = {
        //for (_ <- 0 to 30) {
            val src = new IntSenders(hano.util.Iterable(1,2,3,4,5,6), hano.util.Iterable(7,7,7,7))
            val dst = new IntReceiver(hano.util.Iterable(8,9,10,11))
