...
class RistTest extends org.scalatest.junit.JUnit3Suite {
    ...
    def testTrivial {
        ...
        rx add 4
        rx add 6
        assertEquals(hano.Iter(12,13,5,4,6), hano.Iter.from(out))
    }
...
*/
    def testSignal {
        ...
        a add 7
        b add 35
        assertEquals(hano.Iter(3,9,36,42), hano.Iter.from(out))
    ...
    def testSignal3 {
        ...
        assertEquals(hano.Iter(5,6,6,7,7,8), hano.Iter.from(out))
        a add 7
        assertEquals(hano.Iter(5,6,6,7,7,8,11,12), hano.Iter.from(out))
        b add 35
    ...
    def testTwice {
        ...
        assertEquals(hano.Iter(4,5,35, 8,10,70, 12,15,105, 28,35,35*7), hano.Iter.from(out))
        out.clear
        a add 10 // now, a:1,2,3,7,10
        assertEquals(hano.Iter(14,15,35+10,  40,50,350), hano.Iter.from(out)) // old listner still listen.
        out.clear
        b add 30
        assertEquals(hano.Iter(31,32,33,37,  30,30*2,30*3,30*7,  30+10, 30*10), hano.Iter.from(out))
    ...
    def testToReaction {
        ...
        expect(hano.Iter(1,2,3,9,10,11))(hano.Iter.from(out))
    }
}
