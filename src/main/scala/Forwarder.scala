...
trait Forwarder[A] extends Seq[A] with scala.Proxy {
    ...
    override def generate[B](it: Iter[B]): Seq[B] = around(delegate.generate(it))
    override def replace[B >: A](it: Iter[B]): Seq[B] = around(delegate.replace(it))
    override def replaceRegion[B >: A](n: Int, m: Int, it: Iter[B]): Seq[B] = around(delegate.replaceRegion(n, m, it))
