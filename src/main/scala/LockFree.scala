...
private case class IfFirst[T](_andThen: T => Unit) {
    def Else(_else: T => Unit): _IfFirst[T] = new _IfFirst[T](_andThen, _else)
}

private class _IfFirst[-T](_andThen: T => Unit, _else: T => Unit) extends Function1[T, Unit] {
